...
class Observer {
   ...
   */
  on(name: string, fn: () => void, self = null) {
    if (typeof fn !== 'function') {
      return new Error(
        'Parameter error,The second argument must be a function'
      );
    }

    const eventObj = self || this.event;

    if (Array.isArray(name)) {
      const firstName = name.shift();
      eventObj[firstName] || (eventObj[firstName] = {});
      this.on(
        name.length === 0 ? firstName : name,
        fn,
        name.length === 0 ? eventObj : eventObj[firstName]
      );
    } else {
   ...
   * @param {[]} arg 参数
   */
  trigger(name: string, arg: any, self = null) {
    ...
    if (Array.isArray(name)) {
      ...
      if (eventObj[name]) {
        const eventKeys = Reflect.ownKeys(eventObj[name]);
        eventKeys.forEach((item: any) => {
          if (item === this.eventsFiled) {
            const event = eventObj[name][item].slice();
