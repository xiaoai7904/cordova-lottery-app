<template>
    <q-pull-to-refresh @refresh="refresh" :scroll-target="scrollTarget" :disable="true">
        ...
        <q-card-section v-if="isEmpty && pageListStore.loading">
        ...
        <!-- <q-virtual-scroll ref="virtualListRef" :scroll-target="scrollTarget" :items="pageList"
            :virtual-size="pageList.length" @virtual-scroll="loadMoreItems">
            <template v-slot="{ item, index }">
                <div :key="index">
                    <slot :item="item" :index="index"></slot>
                    <div v-show="pageListStore.loading && index === pageList.length - 1" class="row justify-center q-my-md">
...
export interface Props {
    ...
    pullRefreshDisable?: boolean
    isShowAllLoadText?: boolean
}
...
const props = withDefaults(defineProps<Props>(), {
    ...
    isShowAllLoadText: false
})

// const virtualListRef = ref<any>(null)
const pageListStore = reactive({
    loading: false,
})

const isEmpty = computed(() => !props.pageList.length)
const pages = computed(() => Math.ceil(props.pageInfo.total / 10))

const refresh = async (done: any) => {
    try {
        console.log('refresh....')
        pageListStore.loading = true
        emit('load', 1)
        ...
        pageListStore.loading = false
    }
}

// const loadMoreItems = async (data: any) => {
...
//             return;
//         };
//         if (data.index >= props.pageList.length - 2) {
//             pageListStore.loading = true
//             await props.requestApi({ ...props.requestParams })
//             emit('load')
//             setTimeout(() => {
