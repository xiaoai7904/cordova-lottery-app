<template>
  <div :class="isCertification ? 'real-name' : 'real-name-not'">
    ...
    <div class="real-name-content">
      <div class="skeleton" v-if="privateUserStore.realeNameLoading">
        <van-skeleton title :row="3" />
        <van-skeleton title :row="3" />
      </div>

      <div v-if="isCertification && !privateUserStore.realeNameLoading"
        class="real-name-content-item flex column justify-center">
        <img src="./assets/user.png" alt="user">
        <h1>您已完成实名认证</h1>
        <div class="flex justify-between items-center">
          <span>真实姓名</span>
          <span>{{ privateUserStore.certificationInfo.realName }}</span>
        </div>
        <div class="flex justify-between items-center">
          <span>身份证</span>
          <span>{{ privateUserStore.certificationInfo.idCard }}</span>
        </div>
        <div class="flex justify-between items-center">
          <span>手机号</span>
          <span>{{ privateUserStore.certificationInfo.phone }}</span>
        </div>
      </div>

      <div v-if="!isCertification && !privateUserStore.realeNameLoading" class="form-content">
        <q-form ref="myForm" @submit="onSubmit" class="q-gutter-y-md column">
          <div class="form-content-item">
            <h2>姓名</h2>
            <q-input placeholder="请输入姓名" v-model="privateUserStore.addCertificationInfo.realName" outlined lazy-rules
              :rules="[(val) => (val && val.length > 0) || '请输入姓名']" />
          </div>

          <div class="form-content-item">
            <h2>身份证号码</h2>
            <q-input placeholder="请输入身份证号码" v-model="privateUserStore.addCertificationInfo.idCard" outlined lazy-rules
              :rules="[(val) => (val && val.length > 0) || '请输入身份证号码']" />
          </div>

          <div class="form-content-item">
            <h2>手机号码</h2>
            <q-input placeholder="请输入手机号码" v-model="privateUserStore.addCertificationInfo.phone" outlined lazy-rules
          ...
          <div class="form-content-item">
            ...
            <Uploader @success="uploadSuccess1">
              <img v-if="privateUserStore.addCertificationInfo.image1" :src="privateUserStore.addCertificationInfo.image1"
                alt="avatar" />
          ...
          <div class="form-content-item">
            ...
            <Uploader @success="uploadSuccess2">
              <img v-if="privateUserStore.addCertificationInfo.image2" :src="privateUserStore.addCertificationInfo.image2"
                alt="avatar" />
