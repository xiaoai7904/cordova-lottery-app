<template>
  <div class="football">
    ...
    <van-collapse v-else v-model="model.activeMatch" :border="false">
      <van-collapse-item v-for="(data, index) in privateMatchStore.basketballGroup.data" :name="index" :key="index">
        <template #title>
        ...
        <div class="content-expand">
          ...
          <!--混合投注-->
          <template v-for="item in data" :key="item">
            <LhhBet v-if="model.currEvent == '2'" :data="item"></LhhBet>
          </template>
          <!--大小分-->
          <template v-for="item in data" :key="item">
            <DxfBet v-if="model.currEvent == '3'" :data="item"></DxfBet>
          </template>
          <!--胜负差-->
          <template v-for="item in data" :key="item">
            <SfcBet v-if="model.currEvent == '4'" :data="item"></SfcBet>
          </template>
        </div>
      </van-collapse-item>
    </van-collapse>
...
import { Utils, MATCH_STATUS, XA_DEL_BET, BET_TYPE } from 'src/common'
...
export default defineComponent({
  ...
  setup() {
    ...
    watch(() => privateMatchStore.basketballGroup.data, newValue => {
