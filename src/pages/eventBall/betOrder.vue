<template>
  <div class="bet-order">
    ...
    <div class="bet-order-bottom flex-between van-safe-area-bottom">
      <div class="center">
        <p>共<span class="value">{{ getBetCount(isSingle) }}</span>注<span class="value">{{
          getBetTotalAmount(isSingle) }}</span>元</p>
        <p>预计奖金 <span class="value">{{ getEstimatedBonus(isSingle) }}</span>元</p>
      </div>
...
export default defineComponent({
  ...
  setup(props) {
    const { model, betStore, getBetCount, getBetTotalAmount, getEstimatedBonus, addOrder } = useBet()
    ...
    const addOrderEvent = () => {
      addOrder(props.isSingle, props.betType)
    }

    console.log(betStore)
    return { model, betStore, betOrderStore, clearOrder, delBetEvent, getBetCount, getBetTotalAmount, getEstimatedBonus, addOrderEvent, Utils }
  }
})
</script>
