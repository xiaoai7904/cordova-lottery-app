<template>
    <div class="bet-order">
        ...
        <van-popup v-model:show="betOrderStore.sendOrderPopup" position="bottom"
            :style="{ width: '100vw', height: '100vh' }">
            <SendOrder v-if="betOrderStore.sendOrderPopup" :betType="betType"
                @back="() => betOrderStore.sendOrderPopup = false" />
        </van-popup>
    </div>
</template>

<script lang="ts">
import { useBet, useNotify, useUser, useCustomRouter } from 'src/hook'
import { XA_DEL_BET, Utils, BET_TYPE, RouterNameEnum } from 'src/common'
...
export default defineComponent({
    ...
    setup(props) {
        ...
        const { model, betStore, isSingle, getBetOrder, getBetCount, getBetTotalAmount, getEstimatedBonus, addOrder } = useBet()
        ...
        const checkBalanceTips = () => {
            showComfirmDialog({
                ...
                async confirm() {
                    router.push({ name: RouterNameEnum.RECHARGE })
                },
            });
        }

        const addOrderEvent = () => {
            if (Number(userInfo.value.balance) <= 0) {
                checkBalanceTips()
                return;
            }
